...
<div class="container contenuShow">
	...
	<div class="infosMedia">
		<div class="infosGauche">
			...
			<p><span class="caracteristiquesMedia">ISBN : </span>{{$livre->ISBN}}</p>
			<p><span class="caracteristiquesMedia">Tome : </span>{{$livre->book_tome}}</p>
			<p><span class="caracteristiquesMedia">Saga : </span>{{$livre->book_saga}}</p>
		...
		<div class="infosDroite">
			...
			<div>
				<h2 class="sousTitres">Ecrivains</h2>
				<ul>
				@foreach($ecrivains as $ecrivain)
					<li><a href="{{action('ArtistesController@show', $ecrivain->human_id)}}">{{$ecrivain->artist_nickname}}</a></li>
				@endforeach
				</ul>
			</div>
