...
@section('content')
<div class="container">
	<h1 class="titresPage">Liste des films</h1>

	<div>
		<h2>Filtrer par genres</h2>
		@foreach($genres as $genre)
		<a href="{{route('filmsParGenre', ['id' => $genre->genre_id])}}" class="genre">{{$genre->genre_name}}</a>
		@endforeach
	</div>
		...
		@foreach($medias as $media)
			<div class="listeMedias">
				...
				<div class="elementListe infos">
					<a href="{{action('FilmsController@show', $media->media_id)}}" ><h5>{{$media->media_title}}</h5></a>
					<br />
